...
require_once 'Exception/InvalidArgumentException.class.php';
require_once 'Exception/RuntimeException.class.php';

/**
 * @author vpak
 * @date 2012-11-07 13:14:23
 */
class Miao_Console_Getopt
{
	/**
	...
	*/
	 ...
	 * @throws Miao_Console_Exception_InvalidArgumentException
	 */
	public function __construct( $rules, $argv = null, $getoptConfig = array() )
	{
		if ( !isset( $_SERVER[ 'argv' ] ) )
		{
			...
			throw new Miao_Console_Exception_InvalidArgumentException( $errorDescription );
		}
	 ...
	 *
	 * @param  array $argv
	 * @throws \Zend\Console\Miao_Console_Exception_InvalidArgumentException When not given an array as parameter
	...
	public function addArguments( $argv )
	{
		if ( !is_array( $argv ) )
		{
			throw new Miao_Console_Exception_InvalidArgumentException( "Parameter #1 to addArguments should be an array" );
		}
	...
	public function setArguments( $argv )
	{
		if ( !is_array( $argv ) )
		{
			throw new Miao_Console_Exception_InvalidArgumentException( "Parameter #1 to setArguments should be an array" );
		}
		$this->argv = $argv;
		$this->parsed = false;
		return $this;
	...
	public function getUsageMessage()
	{
		...
		{
			$usage .= sprintf( "%s %s\n", str_pad( $linepart[ 'name' ],
				$maxLen ), $linepart[ 'help' ] );
		}
		return $usage;
	}
	...
	public function setAliases( $aliasMap )
	{
		foreach ( $aliasMap as $flag => $alias )
		{
			...
			$flag = $this->ruleMap[ $flag ];
			if ( isset( $this->rules[ $alias ] ) || isset(
				$this->ruleMap[ $alias ] ) )
			{
				$o = ( strlen( $alias ) == 1 ? '-' : '--' ) . $alias;
				throw new Miao_Console_Exception_InvalidArgumentException( "Option \"$o\" is being defined more than once." );
			}
	...
	public function parse()
	{
		...
		while ( count( $argv ) > 0 )
		{
			...
			elseif ( substr( $argv[ 0 ], 0, 1 ) == '-' && ( '-' != $argv[ 0 ] || count(
				$argv ) > 1 ) )
			{
	...
	protected function _parseSingleOption( $flag, &$argv )
	{
		...
		if ( !isset( $this->ruleMap[ $flag ] ) )
		{
			...
			{
				throw new Miao_Console_Exception_RuntimeException( "Option \"$flag\" is not recognized.", $this->getUsageMessage() );
			}
		...
		switch ( $this->rules[ $realFlag ][ 'param' ] )
		{
			case 'required':
				...
				{
					throw new Miao_Console_Exception_RuntimeException( "Option \"$flag\" requires a parameter.", $this->getUsageMessage() );
				}
				break;
			case 'optional':
	 ...
	 *
	 * @param  int $value
	 * @throws Miao_Console_Exception_RuntimeException
	 * @return void
	 */
	protected function _setNumericOptionValue( $value )
	{
		...
		{
			throw new Miao_Console_Exception_RuntimeException( "Using of numeric flags are deny by configuration" );
		}

		if ( empty( $this->getoptConfig[ 'numericFlagsOption' ] ) )
		{
			throw new Miao_Console_Exception_RuntimeException( "Any option for handling numeric flags are specified" );
		}
	...
	protected function _setSingleOptionValue( $flag, $value )
	{
		...
		if ( is_string( $value ) && !empty( $separator ) && is_string(
			$separator ) && substr_count( $value, $separator ) )
		{
	...
	protected function _checkParameterType( $flag, $param )
	{
		...
		switch ( $type )
		{
			case 'word':
				...
				{
					throw new Miao_Console_Exception_RuntimeException( "Option \"$flag\" requires a single-word parameter, but was given \"$param\".", $this->getUsageMessage() );
				}
				break;
			case 'integer':
				if ( preg_match( '/\D/', $param ) )
				{
					throw new Miao_Console_Exception_RuntimeException( "Option \"$flag\" requires an integer parameter, but was given \"$param\".", $this->getUsageMessage() );
				}
				break;
			case 'string':
			default:
	...
	protected function _addRulesModeZend( $rules )
	{
		foreach ( $rules as $ruleCode => $helpMessage )
		{
			...
			foreach ( $flags as $flag )
			{
				if ( empty( $flag ) )
				{
					throw new Miao_Console_Exception_InvalidArgumentException( "Blank flag not allowed in rule \"$ruleCode\"." );
				}
				if ( strlen( $flag ) == 1 )
				{
					if ( isset( $this->ruleMap[ $flag ] ) )
					{
						throw new Miao_Console_Exception_InvalidArgumentException( "Option \"-$flag\" is being defined more than once." );
					}
				...
				{
					if ( isset( $this->rules[ $flag ] ) || isset(
						$this->ruleMap[ $flag ] ) )
					{
						throw new Miao_Console_Exception_InvalidArgumentException( "Option \"--$flag\" is being defined more than once." );
					}
